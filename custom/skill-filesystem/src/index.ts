...
/** Settings namespace holding the live filter document. */
export const SKILL_FILTERS_SETTINGS_NAMESPACE = settingsNamespace('skill-filters')
...
export interface Config {
  ...
  /**
   * Custom fork: discovery sources to include. Absent means every source
   * (the upstream behavior); a present list filters the built-in and custom
   * roots by their `source` tag before any directory is scanned, so
   * `['project-dsh', 'user-dsh']` skips every other-agent tree.
   */
  includeSources?: SkillSource[]
  /**
   * Custom fork: kebab-case skill names (or `prefix-*` globs) never offered
   * from this provider regardless of source; `['okx-*']` hides a whole
   * foreign agent family without touching its files.
   */
  excludeNames?: string[]
}
...
export function apply(ctx: Context, config: Config = {}): void {
  ...
  // Custom fork: the `skill-filters` settings section overrides both filters
  // live (the settings page edits it). Registration resolves the current
  // document once; the scope watch re-applies on every publish (the provider
  // reads settings.yaml asynchronously, so the first value can land on either
  // side of this registration).
...
export class FileSystemSkillProvider implements SkillProvider {
  ...
  private readonly includeSources: ReadonlySet<string> | undefined
  private readonly excludeNames: readonly string[]
  ...
  constructor(
    ...
    this.includeSources = config.includeSources === undefined || config.includeSources.length === 0
      ? undefined
      : new Set(config.includeSources)
    this.excludeNames = config.excludeNames ?? []
  }

  /** Custom fork: whether one root's source tag survives `includeSources`. */
  private sourceIncluded(source: string): boolean {
    return this.includeSources === undefined || this.includeSources.has(source)
  }

  /** Custom fork: whether one skill name survives `excludeNames` (`prefix-*` globs allowed). */
  private nameIncluded(name: string): boolean {
    for (const pattern of this.excludeNames) {
      if (pattern.endsWith('*') ? name.startsWith(pattern.slice(0, -1)) : name === pattern) return false
    }
    return true
  }

  /**
   * Custom fork: replace both filters from the settings document and
   * invalidate the catalog. Absent fields revert to the composition config.
   * @param includeSources - discovery source whitelist; undefined keeps config.
   * @param excludeNames - name/glob exclusion list; undefined keeps config.
   */
  setFilters(includeSources: readonly string[] | undefined, excludeNames: readonly string[] | undefined): void {
    this.includeSources = includeSources === undefined || includeSources.length === 0
      ? undefined
      : new Set(includeSources)
    if (excludeNames !== undefined) this.excludeNames = excludeNames
  }
  ...
  async list(options: SkillLookupOptions): Promise<SkillCandidate[] | SkillProviderObservation> {
    ...
    for (const root of roots) {
      for (const skill of await discoverRoot(root, this.ctx, this.name)) {
        // Custom fork: name-level exclusion applies to every source alike,
        // after the root-level source filter already skipped whole trees.
        if (this.nameIncluded(skill.name)) candidates.push(skill)
      }
    }
