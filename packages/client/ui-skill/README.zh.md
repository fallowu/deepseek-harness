...
浏览器插件还在设置面板注册了一个技能页(`settings.section`,id `skills`)。该页通过同一条 `skill.list` wire 展示当前会话所属项目的技能目录:每个可用户调用的技能一行,显示名称与路由描述;`modelInvocable: false` 的条目标记“仅用户调用”;提供刷新操作,并有明确的 无会话 / 空目录 / 加载失败 三种状态。目录按项目解析(宿主读取会话头的 `cwd`),因此无会话时页面会说明前置条件,而不是给出错误列表。
