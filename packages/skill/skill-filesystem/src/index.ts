...
export class FileSystemSkillProvider implements SkillProvider {
  ...
  private includeSources: ReadonlySet<string> | undefined
  private excludeNames: readonly string[]
  ...
  /** Whether one root's source tag survives `includeSources`. */
  private sourceIncluded(source: string): boolean {
    return this.includeSources === undefined || this.includeSources.has(source)
  }

  /** Whether one skill name survives `excludeNames` (`prefix-*` globs allowed). */
  ...
  /**
   * Replace both filters from the settings document and invalidate the catalog.
   * Absent fields revert to the composition config.
  ...
  async list(options: SkillLookupOptions): Promise<SkillCandidate[] | SkillProviderObservation> {
    ...
    for (const root of roots) {
      // Root-level source filter: skip whole trees when includeSources is active.
      if (!this.sourceIncluded(root.source)) continue
      for (const skill of await discoverRoot(root, this.ctx, this.name)) {
        // Name-level exclusion applies to every source alike, after the root filter.
